...
node splitting that prvents larger trees from taking advantage of spacial 
locality of the used data (which leads to more cache misses than programs with
a more conventional behavior).

Despite this, speedups for very stressful loads were observed (8 threads, with
each performing 125,000 lookups in parallel):
\begin{center}
\begin{tabular} { | r | c | c | c |}
\multicolumn{4}{|c|}{Search performance (1000000 searches)}\\\hline\hline
K & Time (msec) & Speedup & EDSF \\\hline
seq & \--- & \--- & \--- \\\hline
1 & 1749 & \--- & \--- \\\hline
2 & 823 & \--- & \--- \\\hline
3 & 808 & \--- & \--- \\\hline
...
insertions (e.g. 8 threads rapidly inserting elements) a slowdown is 
observed due to the blocking nature of Java's \texttt{LinkedBlockingQueue}. But
luckily this is a rare use-case and in the case of two threads rapidly
inserting elements a small speedup is observed. This speedup is small enough
however that it does not warrant a data-table.
...
a result of a split would only happen local to each node.
...
The master node has a queue of all of the operations pending for the tree.
...
\begin{tabular}{ | r | c | c | c | }
\hline
\multicolumn{4}{|c|}{Search performance (1000 searches)}\\\hline\hline
K & Time (msec) & Speedup & EDSF \\\hline
seq & \--- & \--- & \--- \\\hline
1&6246 & \--- & \--- \\\hline
2&2876 & \--- & \--- \\\hline
